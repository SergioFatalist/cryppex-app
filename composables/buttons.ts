...
export function useTgButton(props: DefaultButtonProps) {
  ...
  const component = computed(() => {
    return props.to ? NuxtLink : props.href ? "a" : props.type ? "button" : "div";
  });

  const type = computed(() => {
    if (component.value === "button") return props.type || "button";
  });

  const external = computed(() => {
    if (typeof props.to === "string" && props.to.startsWith("http")) return true;
    else if (typeof props.to === "object" && "path" in props.to && props.to.path?.startsWith("http")) return true;
    return false;
  });

  const relationship = computed(() => {
    if (!external.value) return props.rel;
    return props.rel ?? externalDefaultRelationship;
  });

  const target = computed(() => {
    if (!external.value) return props.target;
